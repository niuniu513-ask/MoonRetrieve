...
- BM25 普通检索与前缀检索改为倒排驱动计分，仅排序命中文档；新增 2,000 文档的稀有词与常见词基准及性能复现说明。
- 补充排序、分数、重复查询词、独立计分路径对照、删除及 JSON 往返回归测试；测试总数达到 109 项。
...
- 新增 `Engine::search_snippets` / `search_explained`，并在 CLI 提供 `snippet` / `explain` 命令。
